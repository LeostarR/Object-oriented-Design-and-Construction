...
public class Parser {
    ...
    public Factor parseFactor() {
        ...
        if (lexer.read().equals("(")) { //表达式因子，函数表达式（已在PreStr中套上了一层括号，因此也可以当成表达式因子处理）
            ...
            return parseTri();
        } else if (Objects.equals(lexer.read(), "dx")
                || Objects.equals(lexer.read(), "dy")
                || Objects.equals(lexer.read(), "dz")) {
            char var = lexer.read().charAt(1);
            return parseDer(var);
        } else { //处理幂函数
            return  parsePow();
        }
    ...
    public Expr parseDer(char v) {
        lexer.next();//'('
        lexer.next();//未知，需要判断嵌套括号
        Expr expr = (Expr) parseExpr();
        lexer.next();//取得")"之后的部分
        expr.derExpr(v);
        return expr;
